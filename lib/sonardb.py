...
class sonarDBManager:
    ...
    def create_tables(self):
        with open(
            os.path.join(os.path.dirname(os.path.realpath(__file__)), "db.sqlite"), "r"
        ) as handle:
            sql = handle.read()
    ...
    def check_db_compatibility(self):
        ...
        if dbver != SUPPORTED_DB_VERSION:
            sys.exit(
                "Compatibility error: the given database is not compatible with this version of sonar (Current database version: "
                + str(dbver)
    ...
    def insert_dna_var(self, seqhash, ref, alt, start, end=None):
        ...
        sql = (
            "SELECT varid FROM dna WHERE start = ? AND end = ? AND alt = ? AND ref = ?;"
        )
    ...
    def insert_prot_var(self, seqhash, protein, locus, ref, alt, start, end=None):
        ...
        varid = self.cursor.execute(
            sql, [protein, locus, start, end, alt, ref]
        ).fetchone()["varid"]
    ...
    def count_metadata(self, field):
        sql = (
            "SELECT COUNT(accession) as counts FROM genome WHERE "
            + field
            + " IS NOT NULL AND "
            + field
            + " !=  '';"
    ...
    def match(
        ...
        if include_acc:
            where_clause.append(
                self.get_metadata_in_condition("accession", *include_acc)
            )
            where_vals.extend(include_acc)
        if exclude_acc:
            where_clause.append(
                self.get_metadata_in_condition("accession", *exclude_acc, negate=True)
            )
        ...
        if exclude_lin:
            where_clause.append(
                self.get_metadata_in_condition("lineage", *exclude_lin, negate=True)
            )
        ...
        if exclude_lab:
            where_clause.append(
                self.get_metadata_in_condition("lab", *exclude_lab, negate=True)
            )
        ...
        if include_source:
            where_clause.append(
                self.get_metadata_in_condition("source", *include_source)
            )
            where_vals.extend(include_source)
        if exclude_source:
            where_clause.append(
                self.get_metadata_in_condition("source", *exclude_source, negate=True)
            )
        ...
        if include_collection:
            where_clause.append(
                self.get_metadata_in_condition("collection", *include_collection)
            )
            where_vals.extend(include_collection)
        if exclude_collection:
            where_clause.append(
                self.get_metadata_in_condition(
                    "collection", *exclude_collection, negate=True
                )
            )
        ...
        if include_technology:
            where_clause.append(
                self.get_metadata_in_condition("technology", *include_technology)
            )
            where_vals.extend(include_technology)
        if exclude_technology:
            where_clause.append(
                self.get_metadata_in_condition(
                    "technology", *exclude_technology, negate=True
                )
            )
        ...
        if include_platform:
            where_clause.append(
                self.get_metadata_in_condition("platform", *include_platform)
            )
            where_vals.extend(include_platform)
        if exclude_platform:
            where_clause.append(
                self.get_metadata_in_condition(
                    "platform", *exclude_platform, negate=True
                )
            )
        ...
        if include_chemistry:
            where_clause.append(
                self.get_metadata_in_condition("chemistry", *include_chemistry)
            )
            where_vals.extend(include_chemistry)
        if exclude_chemistry:
            where_clause.append(
                self.get_metadata_in_condition(
                    "chemistry", *exclude_chemistry, negate=True
                )
            )
        ...
        if include_software:
            where_clause.append(
                self.get_metadata_equal_condition("software", include_software)
            )
            where_vals.append(include_software)
        if exclude_software:
            where_clause.append(
                self.get_metadata_equal_condition(
                    "software", exclude_software, negate=True
                )
            )
        ...
        if include_software_version:
            where_clause.append(
                self.get_metadata_equal_condition(
                    "software_version", include_software_version
                )
            )
        ...
        if exclude_software_version:
            where_clause.append(
                self.get_metadata_equal_condition(
                    "software_version", exclude_software_version, negate=True
                )
            )
        ...
        if include_material:
            where_clause.append(
                self.get_metadata_in_condition("material", *include_material)
            )
            where_vals.extend(include_material)
        if exclude_material:
            where_clause.append(
                self.get_metadata_in_condition(
                    "material", *exclude_material, negate=True
                )
            )
        ...
        if min_ct or max_ct:
            where_clause.append(
                self.get_metadata_numeric_condition("ct", min_ct, max_ct)
            )
            if min_ct:
        ...
        if include_zip:
            where_clause.append(
                self.get_metadata_leading_string_condition("zip", *include_zip)
            )
        if exclude_zip:
            where_clause.append(
                self.get_metadata_leading_string_condition(
                    "zip", *exclude_zip, negate=True
                )
            )
        ...
        if include_dates:
            where_clause.append(
                self.get_metadata_date_condition("date", *include_dates)
            )
        if exclude_dates:
            where_clause.append(
                self.get_metadata_date_condition("date", *exclude_dates, negate=True)
            )
        ## submission_date
        if include_submission_dates:
            where_clause.append(
                self.get_metadata_date_condition(
        ...
        if exclude_submission_dates:
            where_clause.append(
                self.get_metadata_date_condition(
                    "submission_date", *exclude_submission_dates, negate=True
                )
            )
        ...
        if include_seqhash:
            where_clause.append(
                self.get_metadata_in_condition("seqhash", *include_seqhash)
            )
            where_vals.extend(include_seqhash)
        if exclude_seqhash:
            where_clause.append(
                self.get_metadata_in_condition("seqhash", *exclude_seqhash, negate=True)
            )
        ...
        if include_profiles:
            ...
            for profile in include_profiles:
                ...
                if len(profile["dna"]) > 0:
                    profile_clause[-1].append(
                        self.get_profile_condition("dna_profile", *profile["dna"])
                    )
                if len(profile["aa"]) > 0:
                    profile_clause[-1].append(
                        self.get_profile_condition("aa_profile", *profile["aa"])
                    )
        ...
        if exclude_profiles:
            ...
            for profile in exclude_profiles:
                ...
                if profile["dna"]:
                    profile_clause[-1].append(
                        self.get_profile_condition(
                            "dna_profile", *profile["dna"], negate=True
                        )
                    )
                if profile["aa"]:
                    profile_clause[-1].append(
                        self.get_profile_condition(
                            "aa_profile", *profile["aa"], negate=True
                        )
                    )
                if len(profile_clause[-1]) > 1:
...
class sonarDB(object):
    ...
    def match(
        ...
        if include_profiles:
            ...
            include_profiles = [
                self.make_profile_explicit(x) for x in include_profiles
            ]  # Fix here
        ...
        include_submission_dates = [
            x for x in submission_dates if not str(x).startswith("^")
        ]
        exclude_submission_dates = [
            x[1:] for x in submission_dates if str(x).startswith("^")
        ]
