...
def parse_args():
	parser = argparse.ArgumentParser(prog="process_fasta.py", description="splits fasta in single entries and stores sequences in files named by seguid")
    ...
    parser.add_argument('--paranoid', help="activates checks on seguid sequence collisions", action="store_true")
	parser.add_argument('fasta', metavar="FILE", help="fasta file containing one or more entries to split", type=str, nargs="+")
