...
from lib import Lineages_UPDATER
...
def parse_args():
    ...
    subparsers = parser.add_subparsers(
        help="detect, store, and screen for mutations in SARS-CoV-2 genomic sequences"
    )
    ...
    parser_add = subparsers.add_parser(
        "add", parents=[general_parser], help="add genome sequences to the database."
    )
    parser_add_input = parser_add.add_mutually_exclusive_group()
    parser_add_input.add_argument(
        "-f",
        "--file",
        metavar="FILE",
        help="fasta file(s) containing DNA sequences to add",
        type=str,
        nargs="+",
        default=[],
    )
    parser_add_input.add_argument(
        ...
        "--dir",
        metavar="DIR",
        help='add all fasta files (ending with ".fasta" or ".fna") from a given directory or directories',
        type=str,
        nargs="+",
        default=None,
    )
    parser_add.add_argument(
        ...
        "--cache",
        metavar="DIR",
        help="use (and restore data from) a given cache (if not set, a temporary cache is used and deleted after import)",
        type=str,
        default=None,
    )
    parser_add.add_argument(
        ...
        "--timeout",
        metavar="INT",
        help="timout for aligning sequences in seconds (default: 600)",
        type=int,
        default=600,
    )
    parser_add.add_argument(
        "--compressed",
        help="compression of input file format ('none', 'gz', 'xz', default: 'auto')",
        choices=["none", "gz", "xz", "auto"],
        default="auto",
    )
    parser_add.add_argument(
        "--force",
        help="force updating of accessions if description or sequence has changed",
        action="store_true",
    )
    parser_add.add_argument(
        "--noprogress", "-p", help="do not show any progress bar", action="store_true"
    )
    parser_add.add_argument(
        "--source",
        help="define a common data source for all genomes",
        type=str,
        default=None,
    )
    parser_add.add_argument(
        "--collection",
        help="define a common data collection for all genomes",
        type=str,
        default=None,
    )
    parser_add.add_argument(
        "--lab", help="define a common lab for all genomes", type=str, default=None
    )
    parser_add.add_argument(
        "--quiet", "-q", help="do not show any output", action="store_true"
    )
    ...
    parser_remove.add_argument(
        "--acc",
        metavar="STR",
        help="define accession(s) of sequences to delete",
        type=str,
        nargs="+",
        default=[],
    )
    parser_remove.add_argument(
        "--file",
        metavar="FILE",
        help="define file containing accession(s) of sequences to delete (one per line)",
        type=str,
        default=None,
    )
    ...
    parser_match.add_argument(
        ...
        "-e",
        metavar="STR",
        help="match genomes not containing the mutation profile",
        type=str,
        action="append",
        nargs="+",
        default=[],
    ...
    parser_match.add_argument(
        "--lineage",
        metavar="STR",
        help="match genomes of the given pangolin lineage(s) only",
        type=str,
        nargs="+",
        default=[],
    )
    parser_match.add_argument(
        "--acc",
        metavar="STR",
        help="match specific genomes defined by acession(s) only",
        type=str,
        nargs="+",
        default=[],
    )
    parser_match.add_argument(
        "--zip",
        metavar="INT",
        help="only match genomes of a given region(s) defined by zip code(s)",
        type=str,
        nargs="+",
        default=[],
    )
    parser_match.add_argument(
        "--date",
        help="only match genomes sampled at a certain sampling date or time frame. Accepts single dates (YYYY-MM-DD) or time spans (YYYY-MM-DD:YYYY-MM-DD).",
        nargs="+",
        type=str,
        default=[],
    )
    parser_match.add_argument(
        "--submission_date",
        help="only match genomes at a certain submission date or time frame. Accepts single dates (YYYY-MM-DD) or time spans (YYYY-MM-DD:YYYY-MM-DD).",
        nargs="+",
        type=str,
        default=[],
    )
    parser_match.add_argument(
        "--lab",
        metavar="STR",
        help="match genomes of the given lab only",
        type=str,
        nargs="+",
        default=[],
    )
    parser_match.add_argument(
        "--source",
        metavar="STR",
        help="match genomes of the given data source only",
        type=str,
        nargs="+",
        default=[],
    )
    parser_match.add_argument(
        "--collection",
        metavar="STR",
        help="match genomes of the given data collection only",
        type=str,
        nargs="+",
        default=[],
    )
    parser_match.add_argument(
        "--technology",
        metavar="STR",
        help="match genomes of the given sequencing technology only",
        type=str,
        nargs="+",
        default=[],
    )
    parser_match.add_argument(
        "--platform",
        metavar="STR",
        help="match genomes of the given sequencing platform only",
        type=str,
        nargs="+",
        default=[],
    )
    parser_match.add_argument(
        "--chemistry",
        metavar="STR",
        help="match genomes of the given sequencing chemistry only",
        type=str,
        nargs="+",
        default=[],
    )
    parser_match.add_argument(
        "--software",
        metavar="STR",
        help="software used for genome reconstruction",
        type=str,
        default=None,
    )
    parser_match.add_argument(
        "--version",
        metavar="STR",
        help="software version used for genome reconstruction",
        type=str,
        default=None,
    )
    parser_match.add_argument(
    ...
    parser_match.add_argument(
        "--min_ct",
        metavar="STR",
        help="minimal ct value of samples resulting genomes are matched to",
        type=float,
        default=None,
    )
    parser_match.add_argument(
        "--max_ct",
        metavar="STR",
        help="maximal ct value of samples resulting genomes are matched to",
        type=float,
        default=None,
    )
    parser_match.add_argument(
        "--seqhash",
        metavar="STR",
        help="match specific genomes with the given seqhash(es)",
        type=str,
        nargs="+",
        default=[],
    )
    ...
    parser_match_g1.add_argument(
        "--ambig",
        help="include ambiguos sites when reporting profiles (no effect when --count is used)",
        action="store_true",
    )
    parser_match_g2 = parser_match.add_mutually_exclusive_group()
    parser_match_g2.add_argument(
        "--only_frameshifts",
        help="show only genomes containing one or more frameshift mutations",
        action="store_true",
    )
    parser_match_g2.add_argument(
        "--no_frameshifts",
        help="show only genomes containing no frameshift mutation",
        action="store_true",
    )
    parser_match.add_argument(
        "--tsv", help="use tsv instead of csv output", action="store_true"
    )
    parser_match.add_argument(
    ...
    parser_restore.add_argument(
        "--acc",
        metavar="STR",
        help="acession(s) whose sequences are to be restored",
        type=str,
        default=[],
        nargs="+",
    )
    parser_restore.add_argument(
        ...
        "-f",
        metavar="STR",
        help="file containing acession(s) whose sequences are to be restored (one accession per line)",
        type=str,
        default=None,
    )

    # create the parser for the "Var2Vcf" command
    parser_var2vcf = subparsers.add_parser(
        "var2vcf",
        parents=[general_parser],
    ...
    parser_var2vcf.add_argument(
        "--acc",
        metavar="STR",
        help="acession(s) whose sequences are to be exported",
        type=str,
        default=[],
        nargs="+",
    )
    parser_var2vcf.add_argument(
        ...
        "-f",
        metavar="STR",
        help="file containing acession(s) whose sequences are to be exported (one accession per line)",
        type=str,
        default=None,
    )
    parser_var2vcf.add_argument(
    ...
    parser_var2vcf.add_argument(
        "--betaV2",
        help="Use in-memory computing for processing (speed up X5 times). WARNING: the function is still experimental/not fully implemented",
        action="store_true",
    )
    # create the parser for the "update" command
    parser_update = subparsers.add_parser(
        "update", parents=[general_parser], help="add or update meta information."
    )
    parser_update_input = parser_update.add_mutually_exclusive_group()
    parser_update_input.add_argument(
        "--pangolin",
        metavar="FILE",
        help="import linegae information from csv file created by pangolin",
        type=str,
        default=None,
    )
    parser_update_input.add_argument(
        "--csv",
        metavar="FILE",
        help="import metadata from a csv file",
        type=str,
        default=None,
    )
    parser_update_input.add_argument(
        "--tsv",
        metavar="FILE",
        help="import metadata from a tsv file",
        type=str,
        default=None,
    )
    parser_update.add_argument(
        "--fields",
        metavar="STR",
        help='if --csv or --tsv is used, define relevant columns like "pango={colname_in_cs} zip={colname_in_cs} date={colname_in_csv}"',
        type=str,
        nargs="+",
        default=None,
    )
    ...
    parser_info.add_argument(
        "--db",
        metavar="DB_DIR",
        help="sonar database directory (optional)",
        type=str,
        default=None,
    )
    ...
    parser_update_anno = subparsers.add_parser(
        "update-lineage-info",
        help="Update lineage information (e.g., lib/linage.all.tsv).",
    )
...
class sonar:
    ...
    def add(
        ...
        with sonardb.sonarCache(cachedir) as cache, sonardb.sonarDBManager(
            self.dbfile
        ) as dbm:
            # db status
            if not quiet:
                    ...
                    for record in SeqIO.parse(handle, "fasta"):
                            ...
                            if genome_data["seqhash"] != seqhash:
                                if not force:
                                    sys.exit(
                                        "database error: "
                                        + acc
                                        + " exists in the database with a different sequence (use --force to allow updating)"
                                    )
                                ...
                                if not force:
                                    sys.exit(
                                        "database error: "
                                        + acc
                                        + " exists in the database with a different description (use --force to allow updating)"
                                    )
                        ...
                        if dbm.seq_exists(seqhash):
                            ...
                            if not os.path.isfile(fasta):
                                unvalid_letters = sorted(
                                    self.db.check_iupac_nt_code(seq)
                                )
                                if unvalid_letters:
                                    print(
                                        "[Skip] input error: "
                                        + acc
                                        + " contains non-IUPAC characters (found: "
                                        + ", ".join(unvalid_letters)
                                        + ")"
                                    )
                                    continue
                                ...
                                sys.exit(
                                    "cache error: sequence hash "
                                    + seqhash
                                    + " exists in cache but refers to a different sequence"
                                )
            ...
            failed = set()
            for status, seqhash in tqdm(
                pool.imap_unordered(self.db.multi_process_fasta_wrapper, to_process),
                ...
                if failed:
                    print(
                        "timeout warning: following genomes were not added to the database since the respective sequence produced an timeout while aligning:",
                        file=sys.stderr,
                    )
                    for f in failed:
            ...
            if print_steps:
                print(msg)
            self.db.import_genome_from_cache(
                cache.dirname,
                to_import,
                msg=msg,
                dbm=dbm,
            ...
            if updates:
                ...
                if print_steps:
                    print(msg)
                for i in tqdm(
                    range(len(to_update)), desc=msg, disable=disable_progressbar
                ):
    ...
    def update_metadata(
        ...
        if pangolin:
            with self.open_file(
                fname, compressed=compressed, encoding="utf-8-sig"
            ) as handle:
            ...
            with self.open_file(fname, compressed=compressed) as handle:
                ...
                for line in lines:
                    acc = line[accCol]
                    if lineageCol and (
                        acc not in updates or "lineage" not in updates[acc]
                    ):
                    ...
                    if ctCol and line[ctCol]:
                        try:
                            ...
                            sys.exit(
                                "metadata error: "
                                + line[ctCol]
                                + " is not a valid ct value (accession: "
                                + acc
                                + ")"
                            )
        with sonardb.sonarDBManager(self.dbfile) as dbm:
    ...
    def var2vcf(self, acc, date, output, cpu, betaV2):
        if betaV2:
            return sonartoVCFV2.export2VCF(
                self.dbfile, acc, date, output, cpu, self.db.refdescr
            )
        else:
            return sonartoVCF.export2VCF(
                self.dbfile, acc, date, output, cpu, self.db.refdescr
            )
...
def process_update_expressions(expr):
    allowed = {
        ...
        "lineage": "lineageCol",
        "date": "dateCol",
        "submission_date": "submission_dateCol",
        "zip": "zipCol",
        "gisaid": "gisaidCol",
        "ena": "enaCol",
        "collection": "collectionCol",
        "technology": "technologyCol",
        "platform": "platformCol",
        "chemistry": "chemistryCol",
        "software": "softwareCol",
        "version": "versionCol",
...
if __name__ == "__main__":
            ...
            for d in args.date:
                ...
                if not regex.match(d):
                    sys.exit(
                        "input error: "
                        + d
                        + " is not a valid date (YYYY-MM-DD) or time span (YYYY-MM-DD:YYYY-MM-DD)."
                    )
    ...
    if args.tool == "update":
        ...
        if args.csv:
            ...
            snr.update_metadata(
                args.csv, **cols, sep=",", pangolin=False, compressed=args.compressed
            )
        elif args.tsv:
            cols = process_update_expressions(args.fields)
            snr.update_metadata(
                args.tsv, **cols, sep="\t", pangolin=False, compressed=args.compressed
            )
        elif args.pangolin:
            snr.update_metadata(
                args.pangolin, pangolin=True, compressed=args.compressed
            )
        else:
    ...
    if args.tool == "var2vcf":
        ...
        print(
            "Runtime (H:M:S): {:0>2}:{:0>2}:{:05.2f}".format(
