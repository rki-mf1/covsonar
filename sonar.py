...
def parse_args():
	...
	general_parser.add_argument('--db', metavar="FILE", help="sonar database to use", type=str, required=True)
	...
	output_parser.add_argument('-o', '--out', metavar="FILE", help="write output to file", type=str, default=None)

	## parser component: sample input
	sample_parser = argparse.ArgumentParser(add_help=False)
	sample_parser.add_argument('--sample', metavar="STR", help="sample accession(s) to consider ", type=str, nargs="+", default = [])
	sample_parser.add_argument('--sample-file', metavar="FILE", help="file containing sample accession(s) to consider (one per line)", type=str, nargs="+", default = [])

	## parser component: property name
	prop_parser = argparse.ArgumentParser(add_help=False)
	prop_parser.add_argument('--name', metavar="STR", help="name of sample property", type=str, required=True)

	## parser component: reference
	ref_parser = argparse.ArgumentParser(add_help=False)
	ref_parser.add_argument('-r', '--reference', metavar="STR", help="reference accession", type=str, default=None)
	...
	## add-prop parser
	parser_addprops= subparsers.add_parser('add-prop', parents=[general_parser, prop_parser], help='Add a sample property to the database.')
	parser_addprops.add_argument('--descr', metavar="STR", help="description of the new property", type=str, required=True)
	parser_addprops.add_argument('--dtype', metavar="STR", help="data type of the new property", type=str, choices=['integer', 'float', 'text', 'date', 'zip'], required=True)
	parser_addprops.add_argument('--qtype', metavar="STR", help="query type of the new property", type=str, choices=['numeric', 'float', 'date', 'text', 'zip'], default=None)
	parser_addprops.add_argument('--default', metavar="VAR", help="default value of the new property (none by default)", type=str, default=None)

	## delete-prop parser
	parser_delprops= subparsers.add_parser('delete-prop', parents=[general_parser, prop_parser], help='Delete a sample property from the database.')
	parser_delprops.add_argument('--force', help="force property to be deleted", action="store_true")

	## match parser
	parser_match = subparsers.add_parser('match', parents=[sample_parser, output_parser, general_parser], help='get mutations profiles for given accessions.')
	...
	## info parser
	parser_info= subparsers.add_parser('info', parents=[general_parser], help='show software and database info')
...
if __name__ == "__main__":
	...
	## import
	if args.tool == "import":
		sonarBasics.import_data(db, fasta=args.fasta, tsv=args.tsv, cols=args.cols, cachedir = args.cache, autodetect=not args.no_autodetect, progress=not args.no_progress, update=not args.no_progress, threads=arg.threads, debug=args.debug)
	...
	## view-prop
		...
		with sonarDBManager(args.db, debug=debug) as dbm:
			...
			cols = ["name", "argument", "description", "data type", "query type", "standard value"]
			rows = []
			for prop in sorted(dbm.properties.keys()):
				dt = dbm.properties[prop]['datatype'] if dbm.properties[prop]['datatype'] != "float" else "decimal number"
				rows.append([])
				rows[-1].append(prop)
				rows[-1].append("--" + prop)
			...
			print()
			print("OPERATORS")
			print("integer, floating point and decimal number data types support the following operators prefixed directly to the respective value without spaces:")
	...
	# restore
		...
		if len(samples) == 0:
			...
			sonarBasics.restore(args.db, *samples, aligned=args.aligned, debug=args.debug)
